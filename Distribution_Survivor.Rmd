...
U = runif(n=N)
t=2;R = ((1-U)**(-Theta[t])-1)/Theta[t]
Y1 = rexp(n=N)
Y2 = rexp(n=N)
X1 = R*Y1/(Y1+Y2)
U1 = (Theta[t]*X1+1)**(-1/Theta[t])
X2 = R*Y2/(Y1+Y2)
U2 = (Theta[t]*X2+1)**(-1/Theta[t])
C = 1-U1-U2+(U1**(-Theta[t])+U2**(-Theta[t]))**(-1/Theta[t])
...
U = runif(n=N)
t=5;R = log((1-Theta[t])/(1-U)+Theta[t])
Y1 = rexp(n=N)
Y2 = rexp(n=N)
U1 = R*Y1/(Y1+Y2)
U2 = R*Y2/(Y1+Y2)
C = 1-U1-U2+U1*U2/(1-Theta[t]*(1-U1)*(1-U2))
